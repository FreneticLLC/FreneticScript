...
#warning Temporary legacy command registration support until all commands have Meta.
    ...
    public static void ShowUsage(CommandQueue queue, CommandEntry entry, bool doError = true, AbstractCommand cmd = null)
    {
        ...
        {
            CommandEntry.InfoOutput(queue, TextStyle.Separate + cmd.Meta.Name + TextStyle.Base + ": " + cmd.Meta.Description);
            CommandEntry.InfoOutput(queue, TextStyle.Commandhelp + "Usage: /" + cmd.Meta.Name + " " + cmd.Meta.Arguments);
            if (cmd.Meta.IsDebug)
            {
                CommandEntry.InfoOutput(queue, "Note: This command is intended for debugging purposes.");
            }
        }
        if (doError)
        {
